...
class OutboundEmailAccountsViewEdit extends ViewEdit
{
    ...
    public function display()
    {
        ...
        if (isset($focus->mail_xoauth2clientsecret)) {
            $mailXoauth2ClientSecret = $focus->mail_xoauth2clientsecret;
        } else {
        ...
        if (isset($focus->mail_xoauth2_token)) {
            $mailXoauth2Token = $focus->mail_xoauth2_token;
        } else {
            LoggerManager::getLogger()->warn('EmailMan view display error: mail xoauth2 token is not set for focus');
        }
        ...
        $this->ss->assign("notify_fromname", $focus->smtp_from_name);
        $this->ss->assign("notify_fromaddress", $focus->smtp_from_addr);
        ...
        if (
            ...
            empty($this->bean) || empty($this->bean->id) || 
            $this->bean->isOwner($GLOBALS['current_user']->id)
        ) {
            if (
                !empty($this->bean) && 
                empty($this->bean->id) && 
                $this->_returnId != $GLOBALS['current_user']->id
            ) {
